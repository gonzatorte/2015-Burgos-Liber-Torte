...
bool transmisionDirection(float n_in, Vector incidentRay, Vector normal, Vector & res){
    float eta = n_in;
...
Vector Shade::shadeRay(Ray &ray, Isect & isect, int level, int weight){
    ...
    if (level + 1 < maxLevel){
        ...
        if ((weight * figure->ktran > minWeight) && (figure->ktran > 0)){
            ...
            bool no_total_ref = transmisionDirection(figure->refrIndex, ray.direction, normal, transDirection);
            if (no_total_ref){
